...
class WavAudioDataSampleStatisticAggregator:
    ...
    def __repr__(self):
        ...
        if len(self.transcription_lengths) > 0:
            ...
            representation += f"min length of transcription: {min_tr_len}\n"
            representation += f"max length of transcription: {max_tr_len}\n"
            representation += f"avg length of transcription: {avg_tr_len:.2f}\n"
        else:
